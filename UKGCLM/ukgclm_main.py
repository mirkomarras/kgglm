...
from pathlm.models.lm.UKGCLM.ukgclm import UKGCLM
from pathlm.models.lm.UKGCLM.lm_utils import _initialise_type_masks, tokenize_augmented_kg, TimingCallback
from pathlm.sampling import KGsampler
from pathlm.sampling.samplers.samplerLP import KGSamplerLinkPrediction
from pathlm.utils import SEED, check_dir, get_data_dir, get_weight_dir, get_root_data_dir

from pathlm.models.lm.UKGCLM.trainer import PathCLMTrainer, PathCLMTrainer_lp, PathPretrainTrainer, \
    PathFinetuneLinkPredictionTrainer, PathFinetuneExplainableRecTrainer
...
def initialize_model_and_update_config(tokenizer, args):
    ...
    if args.pretrain_ckpt:
        ...
        model = UKGCLM.from_pretrained(args.pretrain_ckpt,
        ...
        model = UKGCLM(AutoConfig.from_pretrained(args.model, **config_kwargs))
...
def train(args: argparse.Namespace, tokenizer, tokenized_dataset, kg):
    ...
    if args.task == 'pretrain':
        trainer = PathPretrainTrainer(
            ...
            callbacks=[EarlyStoppingCallback(early_stopping_patience=3),timing_callback]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, default="pretrain", help="{pretrain, finetuneLP, finetuneRec"
                                                                     " end-to-end (originalPEARLM)}")
    ...
    parser.add_argument("--n_seq_infer_lp", type=int, default=30,
                        help="Number of sequences generated for link prediction")
    parser.add_argument("--n_beams", type=int, default=50,
                        help="Number of sequences generated for each user")
    parser.add_argument("--n_beams_lp", type=int, default=30,
                        help="Number of sequences generated for link prediction")
    ...
    parser.add_argument("--validation_interval", type=int, default=3000,
