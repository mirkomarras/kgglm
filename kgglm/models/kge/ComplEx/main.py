...
from kgglm.models.kge.ComplEx.complex import ComplEx
from kgglm.models.kge.ComplEx.parser_complex import parse_args
...
def initialize_model(kg_train,b_size,emb_dim,weight_decay,lr,use_cuda):
    """Define Model"""
    model = ComplEx(emb_dim, kg_train.n_ent, kg_train.n_rel)
...
def train(args):
    ...
    for epoch in iterator:
        t1 = time()
        """Phase 1: CF training"""
        running_loss=train_epoch(model,sampler,optimizer,dataloader,epoch,args=args)
...
if __name__ == '__main__':
    ...
    if args.task=='train':
        ...
        if not args.lp:
            ...
            build_kg_triplets(args.dataset)
            kg_train = get_set_lp(args.dataset, 'train')
        model = ComplEx(args.embed_size,kg_train.n_ent, kg_train.n_rel)
