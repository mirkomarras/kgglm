...
from kgglm.models.kge.TransE.parser_transe import parse_args
from kgglm.models.kge.TransE.transe import TransE
...
def train_epoch(model, sampler, optimizer, dataloader, epoch, args):
    running_loss = 0.0
    model.train()
    for i, batch in tqdm(enumerate(dataloader), unit='batch', position=1, leave=False, total=len(dataloader)):
...
def evaluate_model(model, args, kg_train=None):
    if not args.lp:
        ...
        for b_uids in tqdm(batched_uids, desc="Extract top-k recommendations for each user", position=1, leave=False):
            ...
            history_u = torch.cat([
                torch.full((len(hist_iid),), i, dtype=int) for i, hist_iid in enumerate(users_positives_mask)
            ])
...
def train(args):
    ...
    iterator = tqdm(range(args.epoch), unit='epoch', position=0)
    for epoch in iterator:
        ...
        """
        model.eval()
        test_metrics, topks = evaluate_model(model, args, kg_train=kg_train)
