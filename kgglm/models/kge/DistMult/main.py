...
from kgglm.models.kge.DistMult.distmul import DistMult
from kgglm.models.kge.DistMult.parser_distmult import parse_args
...
def initialize_model(kg_train,b_size,emb_dim,weight_decay,margin,lr,use_cuda):
    """Define Model"""
    model = DistMult(kg_train.n_ent, kg_train.n_rel,margin, emb_dim)
...
if __name__ == '__main__':
    ...
    if args.task=='train':
        ...
        if not args.lp:
            ...
            build_kg_triplets(args.dataset)
            kg_train = get_set_lp(args.dataset, 'train')
        model = DistMult(kg_train.n_ent, kg_train.n_rel, args.margin, args.embed_size)
