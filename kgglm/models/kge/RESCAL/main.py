...
from kgglm.models.kge.RESCAL.rescal import RESCAL
from kgglm.models.kge.RESCAL.parser_rescal import parse_args
...
def initialize_model(kg_train,b_size,emb_dim,weight_decay,margin,lr,use_cuda):
    """Define Model"""
    model = RESCAL(emb_dim,kg_train.n_ent, kg_train.n_rel,margin)
...
def evaluate_model(model,args):
    if not args.lp:
        ...
        for uid in uids:
            ...
            users_positives_mask=[kg_train.ent2ix[pid] for pid in users_positives[uid]]#per problema di ent2ix. no mapping 1:1
...
if __name__ == '__main__':
    ...
    if args.task=='train':
        ...
        if not args.lp:
            ...
            build_kg_triplets(args.dataset)
            kg_train = get_set_lp(args.dataset, 'train')
        model = RESCAL(args.embed_size,kg_train.n_ent, kg_train.n_rel,args.margin)
