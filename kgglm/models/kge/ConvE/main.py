...
from kgglm.models.kge.ConvE.parser_conve import parse_args
from kgglm.data_mappers.mapper_kge import get_watched_relation_idx
from kgglm.models.kge.ConvE.conve import ConvE
...
def initialize_model(kg_train,b_size,emb_dim,weight_decay,lr,use_cuda,args):
    """Define Model"""
    model = ConvE(emb_dim,kg_train.n_ent, kg_train.n_rel,args)
...
def train_epoch(model,optimizer,dataloader,epoch,n_ent,args):
    running_loss = 0.0
    model.train()
    for i, batch in enumerate(dataloader):
        h, t, r = batch[0], batch[1], batch[2]

        #https://github.com/LB0828/conve_reproduce/blob/master/run.py#L86
...
def train(args):
    ...
    if not args.lp:
        ...
        kg_train = get_set_lp(args.dataset, 'train')
    model,optimizer, dataloader=initialize_model(kg_train,args.batch_size,args.embed_size,args.weight_decay,args.lr,args.use_cuda,args)
    ...
    for epoch in iterator:
        ...
        t1 = time()
        """Phase 1: CF training"""
        running_loss=train_epoch(model,optimizer,dataloader,epoch,kg_train.n_ent,args)
        ...
        iterator.set_description('Epoch {} | Binary Cross Entropy Loss: {:.5f}'.format(epoch + 1, running_loss / len(dataloader)))
...
if __name__ == '__main__':
    ...
    if args.task=='train':
        ...
        if not args.lp:
            ...
            build_kg_triplets(args.dataset)
            kg_train = get_set_lp(args.dataset, 'train')
        model=ConvE(args.embed_size,kg_train.n_ent, kg_train.n_rel,args)
