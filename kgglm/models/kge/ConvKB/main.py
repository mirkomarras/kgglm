...
from kgglm.logging.log_helper import logging_config, create_log_id
method_name="ConvKB"
import logging
import torch
import numpy as np
...
from kgglm.models.kge.ConvKB.convkb import ConvKB
from kgglm.models.kge.ConvKB.parser_convkb import parse_args
from kgglm.data_mappers.mapper_kge import get_watched_relation_idx

"""Utils"""
from kgglm.models.kge.utils import get_test_uids, get_log_dir,load_kg,get_users_positives,remap_topks2datasetid, get_users_positives_lp, get_set_lp,metrics_lp, build_kg_triplets


def initialize_model(kg_train,b_size,emb_dim,weight_decay,lr,use_cuda,filters,margin,device):
    """Define Model"""
    model = ConvKB(kg_train.n_ent, kg_train.n_rel,device,margin, emb_dim=emb_dim, n_filters=filters)
...
def evaluate_model(model,args):
    if not args.lp:
        ...
        for uid in uids:
            ...
            scores = []
            for pid in pids_identifiers:
                pids_tensor = torch.IntTensor([kg_train.ent2ix[pid]]).to(args.device)
                score = model.scoring_function(uid_tensor, pids_tensor, rel_tensor)
        ...
        for e1, r in zip(e1_values,r_values):
            ...
            scores=[]
            for e2 in e2_tensor:
                e2=torch.IntTensor([e2]).to(args.device)
                score = model.scoring_function(e1, e2, r)
                scores.append(score.item())
            scores=np.array(scores)
...
def train(args):
    ...
    if not args.lp:
        ...
        kg_train = get_set_lp(args.dataset, 'train')
    model,optimizer,sampler, dataloader=initialize_model(kg_train,args.batch_size,args.embed_size,args.weight_decay,args.lr,args.use_cuda,args.n_filters,args.margin,args.device)
    ...
    for epoch in iterator:
        ...
        iterator.set_description('Epoch {} | SoftPlus Loss: {:.5f}'.format(epoch + 1, running_loss / len(dataloader)))
...
if __name__ == '__main__':
    ...
    if args.task=='train':
        ...
        if not args.lp:
            ...
            build_kg_triplets(args.dataset)
            kg_train = get_set_lp(args.dataset, 'train')
        model=ConvKB(kg_train.n_ent, kg_train.n_rel,args.device,args.margin, emb_dim=args.embed_size, n_filters=args.filters)
        """Setup device (GPU/CPU)"""
        os.environ['CUDA_VISIBLE_DEVICES'] = '0'
        if torch.cuda.is_available():
            device = torch.device("cuda")
