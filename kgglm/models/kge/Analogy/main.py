...
from kgglm.models.kge.Analogy.analogy import Analogy
from kgglm.models.kge.Analogy.parser_analogy import parse_args
...
def initialize_model(kg_train,b_size,emb_dim,weight_decay,lr,use_cuda):
    """Define Model"""
    model = Analogy(emb_dim, kg_train.n_ent, kg_train.n_rel)
...
def train_epoch(model,sampler,optimizer,dataloader,epoch,args):
    ...
    for i, batch in enumerate(dataloader):
        ...
        pos_labels=torch.ones(pos.shape).to(args.device)
        neg_labels=-torch.ones(neg.shape).to(args.device)
        loss = model.loss(pos, neg,pos_labels,neg_labels)
...
if __name__ == '__main__':
    ...
    if args.task=='train':
        ...
        if not args.lp:
            ...
            build_kg_triplets(args.dataset)
            kg_train = get_set_lp(args.dataset, 'train')
        model = Analogy(args.embed_size, kg_train.n_ent, kg_train.n_rel)
