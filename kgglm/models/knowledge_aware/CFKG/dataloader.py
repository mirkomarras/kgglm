...
from kgglm.datasets.dataloader_base import DataLoaderBase
...
class DataLoaderCFKG(DataLoaderBase):
    ...
    def construct_data(self, kg_data):
        # add inverse kg data. now kg_data = concat(kg_data + kg_data with h, t swapped)
        n_relations = max(kg_data['r']) + 1
        inverse_kg_data = kg_data.copy()
        inverse_kg_data = inverse_kg_data.rename({'h': 't', 't': 'h'}, axis='columns') 
        inverse_kg_data['r'] += n_relations # if real rel is "1", then new inverse relation is '1 + n_relation'
