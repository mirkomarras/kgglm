...
from kgglm.knowledge_graphs.kg_ucpr import KnowledgeGraph
from kgglm.models.rl.UCPR.src.env.sp_user_tri_set import kg_based_get_user_triplet_set
from kgglm.models.rl.UCPR.utils import load_labels

from kgglm.knowledge_graphs.kg_macros import INTERACTION, KG_RELATION, MAIN_PRODUCT_INTERACTION, PATH_PATTERN, SELF_LOOP, USER
from kgglm.models.rl.UCPR.utils import TMP_DIR
from kgglm.datasets.datasets_utils import load_embed
...
class BatchKGEnvironment(object):
    ...
    def _get_actions(self, path, done):
        ...
        for r, next_node_id in candidate_acts:
            ...
            #    score = 99999.0
            scores.append(score)
        candidate_idxs = torch.argsort(torch.tensor(scores))[-self.max_acts:]  # choose actions with larger scores
