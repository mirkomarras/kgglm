...
with open(
    os.path.join(current_path, "config_files", "kg_structure_config.json"), "r"
) as kg_config_structure_file:
    kgs_conf_structure = json.load(kg_config_structure_file)
with open(
    os.path.join(current_path, "config_files", "kg_metapaths_config.json"), "r"
) as kg_config_metapaths_file:
...
RELATION_LIST = {
    dset: sum((list(kg_conf[et].keys()) for et in kg_conf), start=[])
    for dset, kg_conf in kgs_conf_structure.items()
}

REV_INTERACTION = {
    dset: REV_PREFIX + dset_interaction
    for dset, dset_interaction in INTERACTION.items()
}
REV_RELATIONS = {
    dset: [REV_PREFIX + rel for rel in dset_relations]
    for dset, dset_relations in RELATION_LIST.items()
}
