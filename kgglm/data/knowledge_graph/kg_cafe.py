...
from kgglm.data.knowledge_graph.kg_base import KnowledgeGraph
from kgglm.data.knowledge_graph.kg_macros import REV_PREFIX, USER
from kgglm.model.rl.CAFE.cafe_utils import DATA_DIR
...
class CAFEKnowledgeGraph(KnowledgeGraph):
    def __init__(self, dataset):
        ...
        # Assume each
        # correspnonds to a unique pair of (head_entity_type, tail_entity_type)!
        ...
        for i in range(1, len(metapath)):
            ...
            for path in paths:
                ...
                if len(next_ids) > sample_sizes[i - 1]:
                    next_ids = np.random.choice(
                        next_ids, size=sample_sizes[i - 1], replace=False
                    )
    ...
    def fast_sample_path_with_target(
        self, mpath_id, user_id, target_id, num_paths, sample_size=100
    ):
        ...
        for i in range(1, mid_level + 1):
            ...
            for fp in forward_paths:
                ...
                if len(next_ids) > sample_size:
                    next_ids = np.random.choice(
                        next_ids, size=sample_size, replace=False
                    )
                for next_id in next_ids:
        ...
        for i in reversed(range(mid_level + 2, path_len + 1)):  # i=l, l-2,..., mid+2
            ...
            for bp in backward_paths:
                ...
                if len(curr_ids) > sample_size:
                    curr_ids = np.random.choice(
                        curr_ids, size=sample_size, replace=False
                    )
                for curr_id in curr_ids:
