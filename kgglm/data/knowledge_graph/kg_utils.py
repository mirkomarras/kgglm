...
from kgglm.data.knowledge_graph.kg_macros import MAIN_PRODUCT_INTERACTION, ROOT_DIR


def get_KG_structures():
    with open(
        os.path.join(
            ROOT_DIR, "kgglm/knowledge_graphs/config_files/kg_structure_config.json"
        ),
        "r",
    ) as f:
...
def get_KG_metapaths():
    with open(
        os.path.join(
            ROOT_DIR, "kgglm/knowledge_graphs/config_files/kg_metapaths_config.json"
        ),
        "r",
    ) as f:
