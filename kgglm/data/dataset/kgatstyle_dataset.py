...
class KGATStyleDataset(Dataset):
    ...
    def __init__(self, args, path, batch_style="list"):
        ...
        self.mode = "cf"
        assert (
            batch_style in self.batch_styles
        ), f"Error: got {batch_style} but valid batch styles are {list(self.batch_styles.keys())}"
        self.path = path
        self.args = args
        ...
        self.train_data, self.train_user_dict = self._load_ratings(
            os.path.join(path, "train.txt")
        )
        self.valid_data, self.valid_user_dict = self._load_ratings(
            os.path.join(path, "valid.txt")
        )
        self.test_data, self.test_user_dict = self._load_ratings(
        ...
        self.kg_data, self.kg_dict, self.relation_dict = self._load_kg(
            os.path.join(path, "kg_final.txt")
        )
    ...
    def _load_kg(self, file_name):
        ...
        if self.args.dataset in ["ml1m", "lfm1m"]:
            kg_np = np.loadtxt(file_name, skiprows=1, dtype=np.int32)
        else:
    ...
    def _print_data_info(self):
        ...
        print(
            f"[n_entities, n_relations, n_triples]=[{self.n_entities}, {self.n_relations}, {self.n_triples}]"
        )
