...
def get_user_negatives(dataset_name: str) -> Dict[int, List[int]]:
    ...
    ikg_ids = set(
        [int(eid) for eid in set(pid2eid.values())]
    )  # All the ids of products in the kg
    uid_negatives = {}
    # Generate paths for the test set
    train_set = get_set(dataset_name, set_str="train")
    valid_set = get_set(dataset_name, set_str="valid")
    for uid in tqdm(
        train_set.keys(), desc="Calculating user negatives", colour="green"
    ):
        uid_negatives[uid] = [
            int(pid)
            for pid in list(set(ikg_ids - set(train_set[uid]) - set(valid_set[uid])))
        ]
...
def get_user_positives(dataset_name: str) -> Dict[int, List[int]]:
    ...
    valid_set = get_set(dataset_name, set_str="valid")
    for uid in tqdm(
        train_set.keys(), desc="Calculating user positives", colour="green"
    ):
...
def load_embed(dataset_name: str, model_name: str, embed_name: str = "TransE"):
    if not os.path.exists(os.path.join(get_model_dir(model_name, "rl"), "embeddings")):
        ...
        raise FileNotFoundError(
            "Embedding folder not found. Please run preprocess_embeddings.py first"
        )

    checkpoint = glob.glob(
        os.path.join(
            get_model_dir(model_name, "rl"),
            "embeddings",
            f"{embed_name}_structured_{dataset_name}.pkl",
        )
    )
    assert (
        len(checkpoint) != 0
    ), f"[Error]: Please first format previously trained {embed_name} embeddings with preprocess_embeddings.py"
