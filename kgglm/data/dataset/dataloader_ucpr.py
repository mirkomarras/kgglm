...
from kgglm.data.knowledge_graph.kg_macros import INTERACTION, MAIN_PRODUCT_INTERACTION
from kgglm.model.rl.UCPR.utils import DATASET_DIR
...
class Dataset(object):
    ...
    def __init__(self, args, set_name="train", word_sampling_rate=1e-4, verbose=False):
        ...
        self.entity_names, self.other_relation_names = (
            list(entity_filename_edict.keys()),
            list(relation_filename_edict.keys()),
        )
    ...
    def infer_kg_structure(self):
        ...
        self.relation2entity[INTERACTION[self.dataset_name]] = MAIN_PRODUCT_INTERACTION[
            self.dataset_name
        ][0]
    ...
    def load_entities(self, entity_filename_edict):
        ...
        for name in entity_filename_edict:
            vocab = [
                x.split("\t")[0] for x in self._load_file(entity_filename_edict[name])
            ][1:]  # Remove header
