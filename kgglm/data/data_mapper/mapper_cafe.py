...
from kgglm.data_mappers.mapper_base import MapperBase
from kgglm.knowledge_graphs.kg_macros import INTERACTION, PRODUCT
from kgglm.utils import check_dir, get_data_dir, get_model_data_dir
...
class MapperCAFE(MapperBase):
    ...
    def write_split_CAFE(self):
        for set in ["train", "valid", "test"]:
            with gzip.open(
                os.path.join(self.output_folder, f"{set}.txt.gz"), "wt"
            ) as set_file:
                ...
                for uid in set_values.keys():
                    ...
                    pids = [
                        self.ratings_pid2new_id[pid] for pid, time in pid_time_tuples
                    ]
                    writer.writerow([uid] + pids)
            set_file.close()

    def map_to_CAFE(self):
        # {0: 'artist', 1:'engineer', ..., 4:'watched'}
        relations_df = pd.read_csv(
            os.path.join(self.input_folder, "r_map.txt"), sep="\t"
        )
        self.rid2entity_name = dict(
            zip(relations_df.id, relations_df.name.apply(lambda x: x.split("_")[-1]))
        )
    ...
    def handle_kg_relations(self):
        ...
        new_rid2relation_name.update(
            {
                rid + offset: relation_name
                for rid, relation_name in self.rid2relation_name.items()
            }
        )
        self.old_rid2new_rid = {
            rid: rid + offset for rid in self.rid2relation_name.keys()
        }
        ...
        new_rid2relation_name.update(
            {
                n_relations + rid: f"rev_{relation_name}"
                for rid, relation_name in new_rid2relation_name.items()
            }
        )

        # Save kg_relations file
        relations_df = pd.DataFrame(
            list(new_rid2relation_name.items()), columns=["id", "name"]
        )
        relations_df.to_csv(
            os.path.join(self.output_folder, "kg_relations.txt.gz"),
            index=False,
            header=False,
            sep="\t",
            compression="gzip",
    ...
    def handle_kg_rules(self):
        ...
        with gzip.open(
            os.path.join(self.output_folder, "kg_rules.txt.gz"), "wt"
        ) as kg_rules_file:
            writer = csv.writer(kg_rules_file, delimiter="\t")
            writer.writerow(
                [main_relation, self.rid2reverse_rid[main_relation], main_relation]
            )  # Special case for main interaction
            for rid in range(
                1, n_relations
            ):  # Start from 1 to skip the main interaction
    ...
    def _extract_ratings_to_global_id(self):
        ...
        user_entities_df = self.all_entities_df[
            self.all_entities_df.local_id.str.contains("user")
        ].copy()
        user_entities_df.loc[:, "name"] = user_entities_df["name"].apply(
            lambda x: x.split("_")[-1]
        )
        self.rating_uid2global_id = dict(
            zip(user_entities_df.name, user_entities_df.global_id)
        )

        # Extract ratings id (pid) to global id
        item2kg_df = pd.read_csv(
            os.path.join(self.input_folder, "i2kg_map.txt"), sep="\t"
        )
        item2kg_df = item2kg_df[["entity", "pid"]]
        item2kg_df = pd.merge(
            item2kg_df,
            self.all_entities_df[
                self.all_entities_df.local_id.str.contains("product_")
            ],
            left_on="entity",
            right_on="name",
        )
        self.rating_pid2global_id = dict(
            zip(item2kg_df.pid.astype(str), item2kg_df.global_id)
        )

    def _load_e_map(self):
        return pd.read_csv(
            os.path.join(self.input_folder, "e_map.txt"),
            sep="\t",
            names=["old_eid", "name", "entity"],
        ).iloc[1:,][["old_eid", "entity"]]
    ...
    def _collect_user_entities(self):
        ...
        self.ratings_uid2new_id = dict(
            zip(user_df.uid.astype(str), user_df.index.astype(str))
        )
        user_df["name"] = "user_" + user_df.uid.astype(str)
        user_df["old_eid"] = np.nan
        return pd.concat(
            [self.all_entities_df, user_df[["local_id", "name"]]], ignore_index=True
        )

    def _collect_product_entities(self, entity2old_eid):
        pid2kg_df = pd.read_csv(
            os.path.join(self.input_folder, "i2kg_map.txt"), sep="\t"
        )
        pid2kg_df["local_id"] = "product_" + pid2kg_df.index.astype(str)
        self.ratings_pid2new_id = dict(
            zip(pid2kg_df.pid.astype(str), pid2kg_df.index.astype(str))
        )
        ...
        return pd.concat(
            [self.all_entities_df, pid2kg_df[["local_id", "name", "old_eid"]]],
            ignore_index=True,
        )

    def _collect_external_entities(self, old_eid2entity):
        self.kg_df = pd.read_csv(
            os.path.join(self.input_folder, "kg_final.txt"), sep="\t"
        )
        ...
        for rid, entity_name in self.rid2entity_name.items():
            unique_entities_by_type = self.kg_df[
                self.kg_df.relation == rid
            ].entity_tail.unique()
            entity_by_type_df = pd.DataFrame({"old_eid": unique_entities_by_type})
            entity_by_type_df["local_id"] = (
                entity_name + "_" + entity_by_type_df.index.astype(str)
            )
            entity_by_type_df["name"] = entity_by_type_df.old_eid.map(old_eid2entity)
            self.all_entities_df = pd.concat(
                [
                    self.all_entities_df,
                    entity_by_type_df[["local_id", "name", "old_eid"]],
                ],
                ignore_index=True,
            )
    ...
    def _write_kg_entities(self):
        ...
        all_entities_df.to_csv(
            os.path.join(self.output_folder, "kg_entities.txt.gz"),
            index=False,
            sep="\t",
            compression="gzip",
        )
    ...
    def collect_triplets(self):
        ...
        interaction_df = pd.DataFrame(
            ...
            columns=["entity_head", "relation", "entity_tail"],
            dtype="int64",
        )
        ...
        triplets_df = pd.concat(
            [triplets_df, interaction_df, other_triplets_df], ignore_index=True
        )

        # Save triplets DataFrame
        output_folder = get_model_data_dir(self.model_name, self.dataset_name)
        triplets_df.to_csv(
            os.path.join(output_folder, "kg_triples.txt.gz"),
            index=False,
            sep="\t",
            compression="gzip",
        )
        ...
        for uid, pid_time_tuples in self.train.items():
            ...
            for pid, _ in pid_time_tuples:
                ...
                interaction_triplets.extend(
                    [
                        [uid_global, main_interaction, pid_global],
                        [pid_global, rev_main_interaction, uid_global],
                    ]
                )
        return interaction_triplets

    def _map_old_eid_to_global_id(self):
        external_entities_df = self.all_entities_df[
            ~self.all_entities_df.local_id.str.contains("user")
        ]
        old_eid2global_id = {
            PRODUCT: dict(
                zip(
                    external_entities_df[
                        external_entities_df.local_id.str.contains("product")
                    ].old_eid.astype(int),
        ...
        for rid, entity_name in self.rid2entity_name.items():
            entities_by_type = self.kg_df[
                self.kg_df.relation == rid
            ].entity_tail.unique()
            entity_by_type_df = pd.DataFrame({"old_eid": entities_by_type.astype(int)})
            entity_by_type_df = entity_by_type_df.merge(
                external_entities_df, on="old_eid", how="left"
            ).dropna()
            old_eid2global_id[entity_name] = dict(
                zip(entity_by_type_df.old_eid, entity_by_type_df.global_id)
            )
    ...
    def _insert_other_entities_to_triplets_df(self, triplets_df):
        ...
        for rid, entity_name in self.rid2entity_name.items():
            ...
            triplets_by_type.entity_head = triplets_by_type.entity_head.map(
                self.old_eid2global_id[PRODUCT]
            )
            triplets_by_type.entity_tail = triplets_by_type.entity_tail.map(
                self.old_eid2global_id[entity_name]
            )
            triplets_by_type.relation = triplets_by_type.relation.map(
                self.old_rid2new_rid
            )
            ...
            rev_triplets_by_type.relation = rev_triplets_by_type.relation.map(
                self.rid2reverse_rid
            )
...
def main():
    ...
    parser.add_argument(
        "--data", type=str, default="lfm1m", help="One of {ML1M, LFM1M}"
    )
    parser.add_argument("--model", type=str, default="cafe", help="")
    parser.add_argument(
        "--train_size",
        type=float,
        default=0.6,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="size of the valid set expressed in 0.x",
    )
