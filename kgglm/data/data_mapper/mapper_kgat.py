...
class MapperKGAT(MapperBase):
    ...
    def _map_items(self) -> None:
        ...
        i2kg_df.to_csv(
            os.path.join(self.output_folder, "item_list.txt"), sep=self.sep, index=False
        )
        self.ratings_pid2new_id: Dict[int, int] = dict(
            zip(i2kg_df.org_id, i2kg_df.remap_id)
        )

    def _map_entities(self) -> None:
        entity_df = pd.read_csv(os.path.join(self.input_folder, "e_map.txt"), sep="\t")[
            ["entity", "eid"]
        ]
        entity_df.rename(columns={"entity": "org_id", "eid": "remap_id"}, inplace=True)
        entity_df.to_csv(
            os.path.join(self.output_folder, "entity_list.txt"),
            sep=self.sep,
            index=False,
        )
    ...
    def _map_users(self) -> None:
        ...
        users_df.to_csv(
            os.path.join(self.output_folder, "user_list.txt"), sep=self.sep, index=False
        )
        self.ratings_uid2new_id: Dict[int, int] = dict(
            zip(users_df.org_id, users_df.remap_id)
        )

    def _map_relations(self) -> None:
        r_map_df = pd.read_csv(os.path.join(self.input_folder, "r_map.txt"), sep="\t")
        r_map_df.rename(
            columns={"id": "remap_id", "kb_relation": "org_id"}, inplace=True
        )
        r_map_df = r_map_df[["org_id", "remap_id"]]
        r_map_df.to_csv(
            os.path.join(self.output_folder, "relation_list.txt"),
            sep=self.sep,
            index=False,
        )

    def _map_kg_final(self) -> None:
        kg_final_df = pd.read_csv(
            os.path.join(self.input_folder, "kg_final.txt"), sep="\t"
        )[["entity_head", "relation", "entity_tail"]]
        kg_final_df.to_csv(
            os.path.join(self.output_folder, "kg_final.txt"),
            sep=self.sep,
            header=False,
            index=False,
        )
    ...
    def write_split_KGAT(self) -> None:
        for set_name in ["train", "valid", "test"]:
            with open(
                os.path.join(self.output_folder, f"{set_name}.txt"), "w"
            ) as set_file:
                ...
                for uid, pid_time_tuples in set_values.items():
                    ...
                    pids = [
                        self.ratings_pid2new_id[int(pid)] for pid, _ in pid_time_tuples
                    ]
