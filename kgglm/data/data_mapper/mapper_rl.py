...
from kgglm.knowledge_graphs.kg_macros import PRODUCT
...
class MapperRL(MapperBase):
    ...
    def map_to_rl(self):
        ...
        user_df.to_csv(
            os.path.join(output_folder, "user.txt.gz"),
            index=False,
            sep="\t",
            compression="gzip",
        )
        self.ratings_uid2new_id = dict(
            zip([str(uid) for uid in list(user_df.uid)], user_df.new_id)
        )
        # Save products entities
        eid2new_id = {}
        pid2kg_df = pd.read_csv(os.path.join(input_folder, "i2kg_map.txt"), sep="\t")
        pid2kg_df.insert(
            0, "new_id", [str(new_id) for new_id in range(pid2kg_df.shape[0])]
        )
        products_df = pid2kg_df[["new_id", "pid"]]
        products_df.to_csv(
            os.path.join(output_folder, "product.txt.gz"),
            index=False,
            sep="\t",
            compression="gzip",
        )
        self.ratings_pid2new_id = dict(
            zip([str(pid) for pid in list(products_df.pid)], products_df.new_id)
        )
        ...
        for rid, entity_name in rid2entity_name.items():
            unique_entities_by_type = list(
                kg_df[kg_df.relation == rid].entity_tail.unique()
            )
            entity_by_type_df = pd.DataFrame(unique_entities_by_type, columns=["eid"])
            entity_by_type_df.insert(
                0,
                "new_id",
                [str(new_id) for new_id in range(entity_by_type_df.shape[0])],
            )
            entity_by_type_df.to_csv(
                os.path.join(output_folder, f"{entity_name}.txt.gz"),
                index=False,
                sep="\t",
                compression="gzip",
            )
            eid2new_id[entity_name] = dict(
                zip(entity_by_type_df.eid, entity_by_type_df.new_id)
            )
        ...
        for rid, relation_name in rid2relation_name.items():
            ...
            triplets_grouped_by_rel.entity_head = (
                triplets_grouped_by_rel.entity_head.map(eid2new_id[PRODUCT])
            )
            triplets_grouped_by_rel.entity_tail = (
                triplets_grouped_by_rel.entity_tail.map(eid2new_id[entity_name])
            )
            ...
            # Save relations
            with gzip.open(
                os.path.join(output_folder, f"{relation_name}.txt.gz"), "wt"
            ) as curr_rel_file:
    ...
    def write_split_rl(self):
        ...
        for set in ["train", "valid", "test"]:
            with gzip.open(
                os.path.join(output_folder, f"{set}.txt.gz"), "wt"
            ) as set_file:
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="pgpr", help="One of {pgpr | ucpr}"
    )
    parser.add_argument(
        "--train_size",
        type=float,
