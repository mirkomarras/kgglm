import json
import os
from kgglm.knowledge_graphs.kg_macros import ROOT_DIR, MAIN_PRODUCT_INTERACTION

def get_KG_structures():
    with open(os.path.join(ROOT_DIR, 'kgglm/knowledge_graphs/config_files/kg_structure_config.json'), 'r') as f:
        KG_STRUCTURES = json.load(f)
    return KG_STRUCTURES

def get_KG_metapaths():
    with open(os.path.join(ROOT_DIR, 'kgglm/knowledge_graphs/config_files/kg_metapaths_config.json'), 'r') as f:
