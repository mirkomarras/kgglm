...
class MapperCAFE(MapperBase):
    ...
    def handle_kg_relations(self):
        ...
        relations_df.to_csv(os.path.join(self.output_folder, "kg_relations.txt.gz"), index=False, header=False, sep="\t",compression="gzip")
    ...
    def _load_e_map(self):
        return pd.read_csv(os.path.join(self.input_folder, "e_map.txt"), sep="\t", names=["old_eid", "name", "entity"]).iloc[1:, ][["old_eid", "entity"]]
    ...
    def _write_kg_entities(self):
        ...
        all_entities_df.to_csv(os.path.join(self.output_folder, "kg_entities.txt.gz"), index=False, sep="\t",compression="gzip")
...
def main():
    ...
    parser.add_argument('--data', type=str, default='lfm1m',
