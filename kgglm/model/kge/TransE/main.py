...
from kgglm.model.kge.TransE.parser_transe import parse_args
from kgglm.model.kge.TransE.transe import TransE
...
def batch_array(arr, batch_size):
    ...
    batched_array = [
        arr[i * batch_size : (i + 1) * args.batch_size]
        for i in range((len(arr) // batch_size) + extra)
    ]
...
def evaluate_model(model, args, kg_train=None):
    if not args.lp:
        ...
        for b_uids in tqdm(
            batched_uids,
            desc="Extract top-k recommendations for each user",
            position=1,
            leave=False,
        ):
            ...
            for uid in b_uids:
                users_positives_mask.append(
                    [kg_train.ent2ix[pid] for pid in users_positives[uid]]
                )

            history_u = torch.cat(
                [
                    torch.full((len(hist_iid),), i, dtype=int)
                    for i, hist_iid in enumerate(users_positives_mask)
                ]
            )
            history_i = torch.LongTensor(
                [x for p_mask in users_positives_mask for x in p_mask]
            )
...
if __name__ == "__main__":
    ...
    if args.task == "train":
        ...
        if not args.lp:
            ...
            build_kg_triplets(args.dataset)
            kg_train = get_set_lp(args.dataset, "train")
        model = TransE(kg_train.n_ent, kg_train.n_rel, args.margin, args.embed_size)
