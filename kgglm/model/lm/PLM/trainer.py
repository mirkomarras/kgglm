...
from kgglm.model.decoding_constraint import (
...
from kgglm.model.lm.PLM.ranker import CumulativeSequenceScoreRanker
...
class PathCLMTrainer(Trainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.ranker = CumulativeSequenceScoreRanker(
            tokenizer,
    ...
    def __generate_topks_withWordLevel(self, model):
        ...
        print(
            "Average topk length:",
            sum(len(v) for v in self.ranker.topk.values())
            / max(len(self.ranker.topk), 1),
        )
        topks, topk_sequences = self.ranker.topk, self.ranker.topk_sequences
        save_topks_items_results(
            self.dataset_name, self.experiment_name, topks, self.ranker.K
        )
        save_topks_paths_results(
            self.dataset_name, self.experiment_name, topk_sequences, self.ranker.K
        )
    ...
    def evaluate(self, model):
        ...
        _, avg_rec_quality_metrics = evaluate_rec_quality(
            self.dataset_name, topks, self.test_set, method_name="PLM-Rec"
        )
