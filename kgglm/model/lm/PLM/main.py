...
from kgglm.model.kge.TransE.main import train as train_transe
from kgglm.model.kge.TransE.parser_transe import parse_args as parse_args_transe
from kgglm.model.lm.PLM.lm_utils import (
    EmbeddingMapper,
    _initialise_type_masks,
    tokenize_augmented_kg,
)
from kgglm.model.lm.PLM.parser import parser_plm_args
from kgglm.model.lm.PLM.plmrec import PLMRec
from kgglm.model.lm.PLM.trainer import PathCLMTrainer
    ...
    if args.pretrain_ckpt:
        ...
        if "plm-rec" in args.model:
            ...
            config = AutoConfig.from_pretrained(
                args.model.split("@")[1], **config_kwargs
            )
            ...
            print(
                f"Model {model_name} initialized with {embedding_file.split('/')[-1]} embeddings of size: {model.transformer.wte.weight.shape}"
            )
        else:
            print(
                "This train script must be use to train exclusively plm-rec@<model-base> models"
            )
            exit(-1)
    return model
...
if __name__ == "__main__":
    ...
    data_dir_mapping = os.path.join(
        args.data_dir, f"{args.dataset}/preprocessed/mapping/"
    )
    ...
    TOKENIZED_DATASET_PATH = os.path.join(
        args.data_dir,
        f"{dataset_name}/{TOKENIZER_TYPE}/{args.task}_{sample_size}_{dataset_hop_size}_tokenized_dataset.hf",
    )
