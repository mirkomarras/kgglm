...
class PLMRec(GPT2LMHeadModel):
    ...
    def __init__(self, config):
        ...
        self.type_embeddings = torch.nn.Embedding(
            num_embeddings=self.num_kg_types, embedding_dim=config.n_embd
        )  # for entities, relations, and special tokens

        self.type_ids_cache = dict()
        self.type_embeds_cache = dict()
        self.idx_mask_cache = dict()

        self.type_ids_row, self.type_embeds_row = self.__init_type_embeddings(
        ...
        self.relation_head = torch.nn.Linear(
    ...
    def __get_type_embeds(self, n_rows, n_cols):
        ...
        return type_ids, self.type_embeddings(
            type_ids.to(self.type_embeddings.weight.device)
        )
    ...
    def forward(
        ...
        if k not in self.type_ids_cache:
            ...
            self.type_ids_cache[k], self.type_embeds_cache[k] = (
                type_ids[:, :-1],
                type_embeds[:, :-1, :],
            )
        ...
        loss = 0.0
        # entity pred mask
        logits_mask = (type_ids != PLMRec.ENTITY_ID)[:, :(-1)].to(
            lm_entity_logits.device
        )
        label_mask = (type_ids != PLMRec.RELATION_ID)[:, 1:(-1)].to(
            lm_entity_logits.device
        )
        ...
        loss += compute_loss(
            lm_entity_logits[:, :(-1), :][logits_mask],
            input_ids[:, 1:(-1)][label_mask],
            self.ent_mask_weight,
        )

        # relation pred mask
        logits_mask = (type_ids != PLMRec.RELATION_ID)[:, 1:(-1)].to(
            lm_entity_logits.device
        )
        label_mask = (type_ids != PLMRec.ENTITY_ID)[:, 1:].to(lm_entity_logits.device)

        loss += compute_loss(
            lm_relation_logits[:, 1:(-1), :][logits_mask],
            input_ids[:, 1:][label_mask],
