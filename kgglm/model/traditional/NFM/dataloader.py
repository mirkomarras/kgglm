...
from kgglm.data.dataset.dataloader_base import DataLoaderBase
...
class DataLoaderNFM(DataLoaderBase):
    ...
    def construct_data(self, kg_data):
        ...
        self.feat_matrix = sp.coo_matrix(
            (feat_data, (feat_rows, feat_cols)), shape=(self.n_items, self.n_entities)
        ).tocsr()
    ...
    def generate_train_batch(self, user_dict):
        batch_user, batch_pos_item, batch_neg_item = self.generate_cf_batch(
            user_dict, self.train_batch_size
        )
    ...
    def generate_valid_batch(self, user_dict):
        batch_user, batch_pos_item, batch_neg_item = self.generate_cf_batch(
            user_dict, self.valid_batch_size
        )
        batch_user_sp = self.user_matrix[batch_user.numpy()]
        batch_pos_item_sp = self.feat_matrix[batch_pos_item.numpy()]
        batch_neg_item_sp = self.feat_matrix[batch_neg_item.numpy()]
    ...
    def generate_test_batch(self, batch_user):
        ...
        batch_user_sp = sp.coo_matrix(
            (user_data, (user_rows, user_cols)), shape=(n_rows, self.n_users)
        ).tocsr()
