...
def save_best(best_metrics, test_metrics, grid):
    ...
    x = (
        sum(test_metrics[OPTIM_HPARAMS_METRIC][-OPTIM_HPARAMS_LAST_K:])
        / OPTIM_HPARAMS_LAST_K
    )
    best_x = (
        sum(best_metrics[OPTIM_HPARAMS_METRIC][-OPTIM_HPARAMS_LAST_K:])
        / OPTIM_HPARAMS_LAST_K
    )
...
def main(args):
    ...
    def can_run(dataset_name):
        if len(os.listdir(BEST_CFG_DIR[dataset_name])) > 0:
            print(
                f"Action required: WARNING {dataset_name} best hyper parameters folder is not empty"
            )
            if not prompt():
                print(
                    "Content not deleted, To run grid search re-run the script and confirm deletion"
                )
                return False
            else:
                shutil.rmtree(BEST_CFG_DIR[dataset_name])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, default=LFM1M, help="One of {ml1m, lfm1m}"
    )
