...
from kgglm.model.rl.PGPR.parser import parser_pgpr_test
from kgglm.model.rl.PGPR.pgpr_utils import (
    load_labels,
    LOG_DATASET_DIR,
    MODEL,
    get_path_pattern,
...
from kgglm.model.rl.PGPR.train_agent import ActorCritic
...
def evaluate_paths(
    ...
    for uid in best_pred_paths:
        ...
        top10_pids = [
            p[-1][2] for _, _, p in sorted_path[:k]
        ...
        if add_products and len(top10_pids) < k:
            ...
            for cand_pid in cand_pids[::-1]:
                if (
                    cand_pid in train_pids
                    or cand_pid in valid_pids
                    or cand_pid in top10_pids
                ):
                    continue
    ...
    evaluate_rec_quality(
        dataset_name, random_topk, test_labels, method_name="Random Baseline"
    )
    mostpop_topk = compute_mostpop_topk(dataset_name, k)
    evaluate_rec_quality(
        dataset_name, mostpop_topk, test_labels, method_name="MostPop Baseline"
    )
