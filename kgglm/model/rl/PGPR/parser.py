...
def parser_pgpr_train():
    ...
    parser.add_argument(
        "--act_dropout", type=float, default=0, help="action dropout rate."
    )
    parser.add_argument(
...
def parser_pgpr_test():
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--dataset", type=str, default="ml1m", help="One of {cloth, beauty, cell, cd}"
    )
    parser.add_argument(
        "--name", type=str, default="train_agent", help="directory name."
    )
    ...
    parser.add_argument(
        "--topk", type=list, nargs="*", default=[25, 5, 1], help="number of samples"
    )
    parser.add_argument(
        "--run_path",
        type=boolean,
    ...
    # For Embeddings
    parser.add_argument(
        "--embed_name", type=str, default="TransE", help="Which kge embedding model?"
    )
    args = parser.parse_args()
