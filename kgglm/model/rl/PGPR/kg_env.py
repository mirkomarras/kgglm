...
from kgglm.datasets.datasets_utils import load_embed, load_kg
from kgglm.knowledge_graphs.kg_macros import SELF_LOOP, USER
from kgglm.knowledge_graphs.kg_utils import (
    KG_RELATION,
    MAIN_PRODUCT_INTERACTION,
    PATH_PATTERN,
)
from kgglm.model.rl.PGPR.pgpr_utils import TMP_DIR, load_labels
...
class BatchKGEnvironment(object):
    def __init__(
        ...
        for embedding in self.embeds:
            if isinstance(self.embeds[embedding], torch.Tensor):
                self.embeds[embedding] = self.embeds[
                    embedding
                ].cpu()  # Move to CPU if it's a torch Tensor
        ...
        u_p_scores = np.dot(
            self.embeds[USER] + self.embeds[main_relation][0],
            self.embeds[main_entity].T,
        )
    ...
    def _get_state(self, path):
        ...
        user_embed = self.embeds[USER][int(path[0][-1]) - 1]  # -1 ale
        ...
        curr_node_embed = self.embeds[curr_node_type][curr_node_id - 1]  # -1 ale
        last_node_embed = self.embeds[last_node_type][last_node_id - 1]  # -1 ale
        ...
        older_node_embed = self.embeds[older_node_type][older_node_id - 1]  # -1 ale
