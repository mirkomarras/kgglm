...
from kgglm.data.dataset.datasets_utils import load_dataset
from kgglm.data.knowledge_graph.kg_macros import KG_RELATION, SELF_LOOP, USER
from kgglm.model.rl.UCPR.src.model.lstm_base.backbone_lstm import KGState_LSTM
from kgglm.model.rl.UCPR.src.model.lstm_base.model_kg import KG_KGE
from kgglm.model.rl.UCPR.src.model.lstm_base.model_kg_pre import KG_KGE_pretrained
...
class UCPR(AC_lstm_mf_dummy):
    def __init__(
        ...
        super().__init__(
            args, user_triplet_set, rela_2_index, act_dim, gamma, hidden_sizes
        )
        ...
        self.dummy_rela = torch.ones(
            max(self.user_triplet_set) * 2 + 1, 1, self.embed_size
        )
        self.dummy_rela = nn.Parameter(self.dummy_rela, requires_grad=True).to(
            self.device
        )
        self.dummy_rela_emb = nn.Embedding(
            max(self.user_triplet_set) * 2 + 1, self.embed_size * self.embed_size
        ).to(self.device)
        self.dataset_name = args.dataset
        self.dataset = load_dataset(args.dataset, TMP_DIR)
    ...
    def _get_next_node_type(self, curr_node_type, relation, next_node_id):
        return KG_RELATION[self.dataset_name][curr_node_type][relation]
    ...
    def bulid_model_reasoning(self):
        ...
        for i in range(self.reasoning_step):
            self.rn_state_tr_query.append(
                nn.Linear(self.embed_size * 2, self.embed_size).cuda()
            )
            self.update_rn_state.append(
                nn.Linear(self.embed_size * 2, self.embed_size).cuda()
            )
            self.rn_query_st_tr.append(
                nn.Linear(self.embed_size * (self.p_hop), self.embed_size).cuda()
            )

            self.rh_query.append(
                nn.Linear(self.embed_size, self.embed_size, bias=False).cuda()
            )
            self.o_r_query.append(
                nn.Linear(self.embed_size, self.embed_size, bias=False).cuda()
            )
            self.v_query.append(
                nn.Linear(self.embed_size, self.embed_size, bias=False).cuda()
            )
            self.t_u_query.append(
                nn.Linear(self.embed_size, self.embed_size, bias=False).cuda()
            )
    ...
    def bulid_mode_user(self):
        self.relation_emb = nn.Embedding(
            len(self.rela_2_index), self.embed_size * self.embed_size
        )

        self.update_us_tr = []
        for hop in range(self.p_hop):
            self.update_us_tr.append(
                nn.Linear(self.embed_size * 2, self.embed_size).cuda()
            )
    ...
    def reset(self, uids=None):
        ...
        for i in range(max(1, self.p_hop)):
            self.memories_h[i] = torch.cat(
                [
                    torch.cat(
                        [
                            self.kg_emb.lookup_emb(
                                u_set[0],
                                type_index=torch.LongTensor([u_set[1]]).to(self.device),
                            )
                            for u_set in self.user_triplet_set[user][i][0]
                        ],
                        0,
                    ).unsqueeze(0)
            ...
            self.memories_r[i] = torch.cat(
                [
                    self.relation_emb(
                        torch.LongTensor(
                            [
                                self.rela_2_index[relation]
                                for relation in self.user_triplet_set[user][i][1]
                            ]
                        ).to(self.device)
                    ).unsqueeze(0)
                    for user in self.uids
            ...
            self.memories_r[i] = self.memories_r[i].view(
                -1, self.n_memory, self.embed_size, self.embed_size
            )

            self.memories_t[i] = torch.cat(
                [
                    torch.cat(
                        [
                            self.kg_emb.lookup_emb(
                            ...
                            for u_set in self.user_triplet_set[user][i][2]
                        ],
                        0,
                    ).unsqueeze(0)
                    for user in self.uids
    ...
    def forward(self, inputs):
        state, res_user_emb, next_enti_emb, next_action_emb, act_mask = (
        ...
        # TODO: which is correct one? scalar or self.scalar?
    ...
    def select_action(self, batch_state, batch_next_action_emb, batch_act_mask, device):
        ...
        next_enti_emb, next_action_emb = (
            batch_next_action_emb[0],
            batch_next_action_emb[1],
        )
        probs, value = self(
            (state_output, res_user_emb, next_enti_emb, next_action_emb, act_mask)
    ...
    def rn_query_st(self, state, relation_embed_dual, rn_step):
        ...
        state = torch.cat([state.squeeze(1)], -1)
        relation_embed_dual = torch.cat([relation_embed_dual.squeeze(1)], -1)
        o_list = []
        for hop in range(self.p_hop):
            ...
            r_v = relation_embed_dual.unsqueeze(1).repeat(
                1, self.memories_t[0].shape[1], 1, 1
            )
    ...
    def update_query_embedding(self, selc_entitiy):
        ...
        for hop in range(self.p_hop):
            tmp_memories_t = torch.cat([self.memories_t[hop], selc_entitiy], -1)
    ...
    def update_path_info_memories(self, up_date_hop):
        ...
        for i in range(max(1, self.p_hop)):
            ...
            self.memories_h[i] = torch.cat(new_memories_h[i], 0).to(self.device)
            self.memories_r[i] = torch.cat(new_memories_r[i], 0).to(self.device)
            self.memories_t[i] = torch.cat(new_memories_t[i], 0).to(self.device)
    ...
    def generate_st_emb(self, batch_path, up_date_hop=None):
        ...
        tmp_state = [
            self._get_state_update(index, path) for index, path in enumerate(batch_path)
        ]

        all_state = torch.cat([ts[3].unsqueeze(0) for ts in tmp_state], 0)
        if len(batch_path[0]) != 1:
            selc_entitiy = torch.cat([ts[0].unsqueeze(0) for ts in tmp_state], 0)
        ...
        # curr_node_embed = torch.cat([ts[0].unsqueeze(0) for ts in tmp_state], 0)
        relation_embed = torch.cat([ts[1].unsqueeze(0) for ts in tmp_state], 0)
        relation_embed_dual = torch.cat([ts[2].unsqueeze(0) for ts in tmp_state], 0)
        ...
        for rn_step in range(self.reasoning_step):
            ...
            if rn_step < self.reasoning_step - 1:
                state_tmp_ = torch.cat([query_state, state_tmp], -1)
        ...
        state_output = state_output.squeeze(
            1
        )  # state_output.squeeze() #results in crash if batch size is 1,
        ...
        res_user_emb = res_user_emb.squeeze(
            1
        )  # res_user_emb.squeeze() #results in crash if batch size is 1
        # furthermore the squeeze is useless, as for normal batches the tensors before squeeze do not have collapsable dimensions
    ...
    def _get_state_update(self, index, path):
        ...
        if len(path) == 1:
            ...
            relation_embed_dual = self.dummy_rela_emb(
                torch.LongTensor([path[0][-1]]).to(self.device)
            )
            relation_embed_dual = relation_embed_dual.view(
            ...
            relation_embed_dual = self.relation_emb(
                torch.LongTensor([self.rela_2_index[last_relation]]).to(self.device)
            )
            relation_embed_dual = relation_embed_dual.view(
                self.embed_size, self.embed_size
            )
        ...
        return [
            curr_node_embed,
            last_relation_embed.squeeze(),
            relation_embed_dual.squeeze(),
            st_emb,
        ]
