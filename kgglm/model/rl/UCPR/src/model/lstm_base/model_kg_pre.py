...
from kgglm.datasets.datasets_utils import load_dataset, load_embed, load_kg
...
class KG_KGE_pretrained(nn.Module):
    def __init__(self, args):
        ...
        self.embeds = load_embed(args.dataset, "UCPR", "TransE")
    ...
    def initialize_relations_embeddings(self, dataset):
        ...
        for relation_name in dataset.other_relation_names:
            value = edict(
                ...
                et_distrib=self._make_distrib(
                    getattr(dataset, relation_name).et_distrib
                ),
            )
        ...
        self.relations[main_rel] = edict(
            et="product",
            et_distrib=self._make_distrib(
                getattr(dataset, "review").product_uniform_distrib
            ),
        )
    ...
    def _entity_embedding(self, key, vocab_size):
        ...
        embed.weight.data = torch.from_numpy(
            np.concatenate(
                (self.embeds[key], np.zeros_like(self.embeds[key])[:1, :]), axis=0
            )[:, : self.embed_size]
        )
    ...
    def _relation_embedding(self, key):
        ...
        if key != SELF_LOOP and key != "padding":
            embed = nn.Parameter(
                torch.from_numpy(
                    np.expand_dims(self.embeds[key][0], axis=0)[:, : self.embed_size]
                )
            )
        else:
