...
class AC_lstm_mf_dummy(nn.Module):
    ...
    def forward(self, inputs):
        state, batch_next_action_emb, act_mask = (
    ...
    def update_path_info(self, up_date_hop):
        ...
        self.prev_state_h = torch.cat(new_prev_state_h, 1).to(self.device)
        self.prev_state_c = torch.cat(new_prev_state_c, 1).to(self.device)
    ...
    def generate_act_emb(self, batch_path, batch_curr_actions):
        return torch.cat(
            [
                self._get_actions(index, actions_sets[0], actions_sets[1]).unsqueeze(0)
    ...
    def update(self, optimizer, env_model, device, ent_weight, step):
        ...
        optimizer.zero_grad()
        loss.backward()
        if self.args.grad_check and step % 50 == 0:
