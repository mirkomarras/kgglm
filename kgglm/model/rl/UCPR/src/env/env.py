...
from kgglm.data.dataset.datasets_utils import load_embed
from kgglm.data.knowledge_graph.kg_macros import (
    INTERACTION,
    KG_RELATION,
...
from kgglm.model.rl.UCPR.src.env.sp_user_tri_set import kg_based_get_user_triplet_set
from kgglm.model.rl.UCPR.utils import TMP_DIR, load_labels
...
class PATH_PTN(object):
    def __init__(self, dataset_name):
        ...
        for pattern_id in sorted(PATH_PATTERN[dataset_name].keys()):
    ...
    def _kg_has_pattern(self, path):
        ...
        return (
            pattern[0] == SELF_LOOP
            and (pattern[1] == INTERACTION[self.dataset_name])
            and pattern[2] != SELF_LOOP
            and pattern[3] != SELF_LOOP
        )
...
class BatchKGEnvironment(object):
    def __init__(self, args, dataset, max_acts, max_path_len=3, state_history=1):
        ...
        # P2 means KnowledgeGraph dataset environment
            ...
            self.next_type = lambda curr_node_type, relation, next_node_id: KG_RELATION[
                self.dataset_name
            ][curr_node_type][relation]
            self.get_user_triplet = kg_based_get_user_triplet_set

        self.set_UC_view()
        self.PATH_PTN = PATH_PTN(self.dataset_name)

    def next_type(self, curr_node_type, relation, next_node_id):
    ...
    def load_dataset_emb(self, args, dataset_str):
        ...
        self.user_list = [
            user for user in list(self.kg(USER).keys()) if user in self.core_user_list
        ]

        # taken from original implementation
        self.args.sp_user_filter = self.total_user_list[:800]

        self.embeds = load_embed(dataset_str, "PGPR")
    ...
    def set_UC_view(self):
        if self.args.non_sampling:
            self.user_triplet_set = [
                user
                for user in list(self.kg(USER).keys())
                if user in self.core_user_list
            ]
            return

        user_triplet_path = "{}/triplet_set_{}.pickle".format(
            TMP_DIR[self.dataset_name], self.args.name
        )  # self.args.save_model_dir, self.args.name)
        if os.path.exists(user_triplet_path):
            ...
            if self.args.envir == "p1":
                self.user_triplet_set = self.get_user_triplet(
                    self.args, self.kg, self.user_list, self.p_hop, self.n_memory
                )
            else:
                self.user_triplet_set = self.get_user_triplet(
                    self.args, self.kg, self.user_list, self.p_hop, self.n_memory
    ...
    def select_user_th(self):
        ...
        user_counter_dict = sorted(
            user_counter_dict.items(), key=lambda x: x[1], reverse=True
        )
        self.user_counter_dict = user_counter_dict
        self.core_user_list = [
            user_set[0] for user_set in user_counter_dict
        ]  # APPLY FILTERING TOPK IF NEEDED, not used in the original final version of UCPR #[:self.user_top_k]]
    ...
    def _get_actions(self, path, done):
        ...
        for r in relations_nodes:
            ...
            next_node_ids = [
                n_set[1]
                for n_set in next_node_set
                if (n_set[0], n_set[1]) not in visited_nodes
            ]
        ...
        for r, next_node_id in candidate_acts:
            ...
            if next_node_type == USER:
                ...
                src_embed = (
                    user_embed + self.embeds[main_product][0] + self.embeds[r][0]
                )
            score = torch.matmul(src_embed, self.embeds[next_node_type][next_node_id])
            ...
            #    score = 99999.0
            scores.append(score)
        candidate_idxs = torch.argsort(torch.tensor(scores))[
            -self.max_acts :
    ...
    def _kg_get_reward(self, path):
        ...
        if curr_node_type == main_entity:
            ...
            score = 0
            uid = path[0][-1]
            if curr_node_id in self.kg(USER, uid)[main_relation]:
                score += 1
            else:
                score += 0.0
