...
from kgglm.model.rl.UCPR.utils import EVALUATION, EVALUATION_2, SAVE_MODEL_DIR
...
def parameter_path(args):
    ...
    # log_dir_fodder = (
    #     f"{args.name}_g_aiu_{args.att_core}_{args.item_core}_{args.user_core}"
    # )
...
def parameter_path_th(args):
    ...
    args.log_dir = "{}/{}/{}".format(
        EVALUATION_2[args.dataset], args.model, log_dir_fodder
    )
    if not os.path.isdir(args.log_dir):
        os.makedirs(args.log_dir)

    args.save_model_dir = "{}/{}/{}".format(
        SAVE_MODEL_DIR[args.dataset], args.model, log_dir_fodder
    )
    if not os.path.isdir(args.save_model_dir):
        os.makedirs(args.save_model_dir)

    args.pretrained_dir = "{}/{}/{}".format(
        EVALUATION[args.dataset], "pretrained", "emb_szie_" + str(args.embed_size)
    )
    if not os.path.isdir(args.pretrained_dir):
        os.makedirs(args.pretrained_dir)
