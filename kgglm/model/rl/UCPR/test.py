...
from kgglm.model.rl.PGPR.pgpr_utils import get_path_pattern, load_labels
from kgglm.model.rl.UCPR.para_setting import parameter_path, parameter_path_th
from kgglm.model.rl.UCPR.parser import parse_args
from kgglm.model.rl.UCPR.src.model.get_model.get_model import (
    Memory_Model,
    KGEnvironment,
)
from kgglm.model.rl.UCPR.utils import (
    LOG_DATASET_DIR,
    ...
    for uid in pred_paths.keys():
        ...
        for pid in curr_pred_paths.keys():
            ...
            for i, curr_path in enumerate(curr_pred_paths_for_pid):
                ...
                new_path_score = (float(path_score) - min_score) / (
                    max_score - min_score
                )
...
def evaluate(topk_matches, test_user_products, no_skip_user, dataset_name):
    ...
    for metric, values in metrics.items():
        ...
        avg_metric_value = (
            np.mean(values) * 100 if metric == "ndcg_other" else np.mean(values)
        )
        n_users = len(values)
        print(
            "Overall for noOfUser={}, {}={:.4f}".format(
                n_users, metric, avg_metric_value
            )
        )
        print("\n")
...
def batch_beam_search(args, env, model, uids, device, topk=[25, 5, 1]):
    ...
    for hop in range(3):
        ...
        try:
            ...
            probs, _ = model(
                (state_tensor, batch_next_action_emb, actmask_tensor)
        ...
        for row in range(topk_idxs.shape[0]):
            ...
            for idx, p in zip(topk_idxs[row], topk_probs[row]):
                ...
                if relation == SELF_LOOP:
                    ...
                    if args.envir == "p1":
                        next_node_type = KG_RELATION[args.dataset][path[-1][1]][
                            relation
                        ]  # KG_RELATION[path[-1][1]][relation]
                    else:
                        next_node_type = KG_RELATION[args.dataset][path[-1][1]][
                            relation
                        ]  # env.et_idx2ty[next_node_id]
...
def predict_paths(args, policy_file, path_file, train_labels, test_labels, pretest):
    ...
    test_uids = [
        uid for uid in test_uids if uid in train_labels and uid in env.user_list
    ]
    ...
    while start_idx < len(test_uids):
        ...
        paths, probs = batch_beam_search(
            args, env, model, batch_uids, args.device, topk=args.topk
        )
        ...
        if times % 50 == 0:
            ...
            fail_batch = (
                f"'batch_uids = ', {fail_uids}, {str(start_idx)}, {str(end_idx)}"
            )
            args.logger.info(fail_batch)

        if pretest == 1 and times >= 100:
            break
...
def evaluate_paths(
    ...
    avg_precision, avg_recall, avg_ndcg, avg_hit, invalid_users = evaluate(
        pred_labels, test_labels, no_skip_user, dataset_name
    )
...
def get_path_pattern_weight(path_pattern_name, pred_uv_paths):
...
def test(args, train_labels, valid_labels, test_labels, best_recall, pretest=1):
    ...
    policy_file = (
        args.policy_path
    )  # args.save_model_dir + '/policy_model_epoch_{}.ckpt'.format(35)#args.eva_epochs)
    path_file = os.path.join(
        TMP_DIR[args.dataset],
        "policy_paths_epoch{}_{}.pkl".format(args.eva_epochs, args.topk_string),
    )  # args.save_model_dir + '/' + 'pre' + str(pretest) + 'policy_paths_epoch{}_{}.pkl'.format(args.eva_epochs, args.topk_string)
    ...
    if args.run_path or not os.path.exists(path_file):
        predict_paths(
            args, policy_file, path_file, train_labels, test_labels, pretest
        )  # predict_paths(policy_file, path_file, args)
    if args.save_paths or args.run_eval():
        pred_paths, scores = extract_paths(
            args.dataset, path_file, train_labels, valid_labels, test_labels
        )
    if args.run_eval:
        evaluate_paths(
            TOP_N_LOGGING,
