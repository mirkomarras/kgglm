...
from kgglm.data.knowledge_graph.kg_macros import INTERACTION
from kgglm.logging.log_helper import get_logger
from kgglm.model.rl.UCPR.data_loader import ACDataLoader
...
from kgglm.model.rl.UCPR.utils import HPARAMS_FILE, TMP_DIR
...
def pretrain_set(args, env):
    if args.load_pretrain_model:
        ...
        if args.dataset in INTERACTION:
            for name, child in model.named_children():
                # print('name = ', name)
                if (
                    name in para_meter
    ...
    if not hasattr(args, "kg_user_filter"):
        args.kg_user_filter = ""
    args.core_user_list = ""
    args.sp_user_filter = ""
    logger.info(args)
    ...
    for epoch in range(0, args.epochs + 1):
        ...
        for split_name, dataloader in splits_to_compute:
            ...
            while dataloader.has_next():
                ...
                env.reset(
                    epoch, batch_uids, training=True
                )  # numpy array of [bs, state_dim]
                ...
                while not env._done:
                    ...
                    batch_next_action_emb = model.generate_act_emb(
                        env._batch_path, env._batch_curr_actions
                    )
                    batch_act_idx = model.select_action(
                        batch_emb_state,
                        batch_next_action_emb,
                        batch_act_mask,
                        args.device,
                    )  # int
        ...
        ### END of epoch ###
        if epoch % 10 == 0:
            policy_file = "{}/policy_model_epoch_{}.ckpt".format(
                TMP_DIR[args.dataset], epoch
            )
