...
TMP_DIR = {dset: f"{DATASET_DIR[dset]}/tmp" for dset in INTERACTION}

SAVE_MODEL_DIR = {dset: f"{LOG_DATASET_DIR[dset]}/save" for dset in INTERACTION}
EVALUATION = {dset: f"{LOG_DATASET_DIR[dset]}/eva_pre" for dset in INTERACTION}
EVALUATION_2 = {dset: f"{LOG_DATASET_DIR[dset]}/eval" for dset in INTERACTION}
CASE_ST = {dset: f"{LOG_DATASET_DIR[dset]}/case_st" for dset in INTERACTION}
TEST = {dset: f"{LOG_DATASET_DIR[dset]}/test" for dset in INTERACTION}

# Label files.
LABELS = {
    dset: (
        TMP_DIR[dset] + "/train_label.pkl",
        TMP_DIR[dset] + "/valid_label.pkl",
        TMP_DIR[dset] + "/test_label.pkl",
    )
...
def ensure_dataset_name(dataset_name):
    if dataset_name not in INTERACTION:
