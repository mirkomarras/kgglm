...
from kgglm.model.rl.CAFE.data_utils import OnlinePathLoader
...
def train(args):
    ...
    dataloader = OnlinePathLoader(
        args.dataset, args.batch_size, topk=args.topk_candidates
    )
    metapaths = dataloader.kg.metapaths

    kg_embeds = load_embed(args.dataset, "TransE") if train else None
    model = create_symbolic_model(
        args, dataloader.kg, train=True, pretrain_embeds=kg_embeds
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        while dataloader.has_next():
            ...
            optimizer.zero_grad()
            reg_loss, rank_loss = model.forward(metapaths[mpid], pos_paths, neg_pids)
            ...
            if steps % args.steps_per_checkpoint == 0:
                ...
                logging.info(
                    "Epoch/Step: {:02d}/{:08d} | ".format(epoch, steps)
                    + "LR: {:.5f} | ".format(lr)
        ...
        if epoch % args.save_interval == 0:
            torch.save(
                model.state_dict(),
                f"{args.weight_dir_ckpt}/symbolic_model_epoch{epoch}.ckpt",
            )

    torch.save(
        model.state_dict(), f"{args.weight_dir}/symbolic_model_epoch{args.epochs}.ckpt"
    )
