...
from kgglm.data.dataset.datasets_utils import get_user_positives
from kgglm.evaluation.eval_metrics import (
    evaluate_rec_quality,
    print_rec_quality_metrics,
)
from kgglm.evaluation.eval_utils import (
    save_topks_items_results,
    save_topks_paths_results,
)
from kgglm.data.knowledge_graph.kg_cafe import USER
...
from kgglm.model.rl.CAFE.data_utils import KGMask
from kgglm.model.rl.CAFE.parser import parse_args
from kgglm.model.rl.CAFE.symbolic_model import create_symbolic_model
...
class MetaProgramExecutor(object):
    ...
    def execute(
        self, program, uid, excluded_pids=None, adaptive_topk=False, manual_topk=5
    ):
        ...
        while queue:  # queue is not empty
            ...
            entity_vecs = self.symbolic_model.embedding(
                node.entity
            )  # tensor [vocab, d]
            scores = torch.matmul(
                node.data["vec"], entity_vecs.t()
            )  # tensor [1, vocab]
            ...
            for path, value, ep, mp in node.parent.data["paths"]:
                ...
                valid_ids = self.kg_mask.get_ids(
                    node.parent.entity, path[-1], node.relation
                )
        ...
        def _postorder(node, msgs):
            ...
            if not node.has_children():
                if hide_branch and msg[2] == 0:
                    return
                str_msgs = [
                    "({},{},{})".format(msg[0], msg[1], msg[2]) for msg in new_msgs
                ]
                print("  ".join(str_msgs))
                return
...
def create_heuristic_program(
    metapaths, raw_paths_with_scores, prior_count, sample_size
):
...
def run_program(args):
    ...
    for uid in test_labels:
        ...
        for path_info in paths:
            ...
            reconstructed_path.extend(
                [
                    (relations[i], path_types[i], path_info[0][i + 1])
    ...
    rec_quality_metrics, avg_rec_quality_metrics = evaluate_rec_quality(
        args.dataset, topks, test_labels, k
    )
