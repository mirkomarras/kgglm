...
class EntityEmbeddingModel(nn.Module):
    def __init__(self, entity_info, embed_size, init_embed=None):
        ...
        for name in entity_info:
            ...
            embed = nn.Embedding(
                info["vocab_size"] + 1, self.embed_size, padding_idx=-1, sparse=False
            )
            initrange = 0.5 / self.embed_size
            weight = torch.FloatTensor(
                info["vocab_size"] + 1, self.embed_size
...
def create_symbolic_model(args, kg, train=True, pretrain_embeds=None):
    ...
    entity_embed_model = EntityEmbeddingModel(
        entity_info, args.embed_size, init_embed=pretrain_embeds
    )
    ...
    if train:
        ...
        pretrain_sd = torch.load(
            args.symbolic_model, map_location=lambda storage, loc: storage
        )
