...
from kgglm.model.knowledge_aware.KGAT.dataloader import KGATLoader
from kgglm.model.knowledge_aware.KGAT.KGAT import KGAT
from kgglm.model.knowledge_aware.KGAT.parser import parse_args
...
def train_epoch(model, data_loader, mode, epoch, args):
    ...
    for iter, batch_data in enumerate(data_loader, 1):
        ...
        if (iter % args.print_every) == 0:
            logging.info(
                f"CF Training: Epoch {epoch:04d} Iter {iter:04d} / {len(data_loader):04d} | "
                f"Iter Loss {batch_loss.item():.4f} | "
                f"Iter Mean Loss {total_loss / iter:.4f}"
            )
    return (
        total_loss / len(data_loader),
        total_base_loss / len(data_loader),
        total_kge_loss,
        total_reg_loss / len(data_loader),
    )
...
def train(args):
    ...
    dataset_obj = KGATLoader(
        args=args, path=get_model_data_dir(args.model_type, args.dataset)
    )
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        avg_loss, avg_base_loss, avg_kge_loss, avg_reg_loss = train_epoch(
            model, cf_data_loader, "rec", epoch, args
        )
        ...
        if args.use_kge:
            ...
            _, _, avg_kge_loss, avg_reg_loss = train_epoch_kge(
                model, dataset_obj, "kge", epoch, args
            )
