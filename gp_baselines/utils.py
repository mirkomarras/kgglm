...
import torch
from pathlm.evaluation.eval_metrics import ndcg_at_k,mmr_at_k
...
def get_set_lp(dataset):
    if dataset == 'ml1m' or dataset == 'lfm1m':
        path = os.path.join('data', dataset, 'preprocessed', 'kg_test.txt')
    else:
        path = os.path.join('data', dataset, 'kg_test.txt')
...
def get_kg_positives_and_tokens_ids_lp(dataset_name: str, tokenizer) -> Tuple[Dict[int, List[int]], Dict[int, List[int]]]:
    ...
    kg_train = load_kg_lp(dataset_name, 'train') # it contain train and valid
