...
def train(args):
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        if epoch % args.save_interval == 0 or epoch == args.epoch - 1:
            torch.save(model.state_dict(), os.path.join(args.weight_dir_ckpt,f'{args.model_type}_epoch_{epoch}_bs{args.batch_size}_lr{args.lr}.pth'))
    # Final model save and cleanup
    torch.save(model.state_dict(), os.path.join(args.weight_dir,f'{args.model_type}_epoch_{epoch}_bs{args.batch_size}_lr{args.lr}.pth'))
