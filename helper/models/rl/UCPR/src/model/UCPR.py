...
from helper.datasets.datasets_utils import load_dataset
from helper.knowledge_graphs.kg_macros import KG_RELATION, SELF_LOOP, USER
from helper.models.rl.UCPR.src.model.lstm_base.backbone_lstm import \
    KGState_LSTM
from helper.models.rl.UCPR.src.model.lstm_base.model_kg import KG_KGE
from helper.models.rl.UCPR.src.model.lstm_base.model_kg_pre import \
    KG_KGE_pretrained
from helper.models.rl.UCPR.src.model.lstm_base.model_lstm_mf_emb import \
    AC_lstm_mf_dummy
from helper.models.rl.UCPR.utils import PADDING, TMP_DIR
...
class UCPR(AC_lstm_mf_dummy):
    ...
    def reset(self, uids=None):
        ...
        for i in range(max(1,self.p_hop)):
            ...
            self.memories_h[i] = torch.cat([torch.cat([self.kg_emb.lookup_emb(u_set[0], type_index = torch.LongTensor([u_set[1]]).to(self.device))
                                 for u_set in self.user_triplet_set[user][i][0]], 0).unsqueeze(0) for user in self.uids], 0)
            
            self.memories_r[i] = torch.cat([self.relation_emb(torch.LongTensor([self.rela_2_index[relation] 
            ...
            self.memories_t[i] = torch.cat([torch.cat([self.kg_emb.lookup_emb(u_set[0], type_index = torch.LongTensor([u_set[1]]).to(self.device))
    ...
    def generate_act_emb(self, batch_path, batch_curr_actions):
        ...
        enti_emb = torch.cat([action_set[0].unsqueeze(0) for action_set in all_action_set], 0)
        next_action_state = torch.cat([action_set[1].unsqueeze(0) for action_set in all_action_set], 0)
